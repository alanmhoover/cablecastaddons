...
conn.request('GET',f'{ccapi.API}shows/search/advanced/{ccapi.DL_SEARCH}')
r1 = conn.getresponse()
...
print(r1.status, r1.reason)
if r1.status == 200:
    with open(f"{ccapi.OUTPUT_DIRECTORY}{showid}{filename}.scc",mode="wb") as outfile:
...
print(r1.status, r1.reason)
if r1.status == 200:
    with open(f"{ccapi.OUTPUT_DIRECTORY}{showid}{filename}.{extension}",mode="wb") as outfile:
        outfile.write(r1.read())
